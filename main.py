...
def main():
    ...
    try:
        # --- NEW: Initialize Firestore memory with persistent session ID ---
        # Use a fixed session ID based on persona to maintain conversation history
        session_id = "eidoid-main-session"
        print(f"[MAIN] Starting new session with ID: {session_id}")
        initialize_firestore_memory(session_id, persona_key=state.active_persona_key)
        
        # --- NEW: Turn LEDs off at startup just in case ---
        led_controller.turn_off()
        while True:
            if state.current_state == state.RobotState.SLEEPING:
                # --- NEW: Turn off LEDs during sleep ---
                led_controller.turn_off()
                ...
                # --- NEW: Start pulsing LEDs when waking up ---
                ...
                # --- NEW: Ensure LEDs are off before resetting state ---
                led_controller.turn_off()
